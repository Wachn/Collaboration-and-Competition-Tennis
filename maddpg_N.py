...
class MADDPG:
    ...
    def act(self, states, noise=1.0):
        ...
        for agent, obs in zip(self.maddpg_agent, states):
            ...
            self.noise.level()
            action = action + self.decay_noise * self.noise.state * noise
    ...
    def step(self, experiences, agent_number, logger):
        ...
        # torch.nn.utils.clip_grad_norm_(agent.critic_local.parameters(), 1, 2)
        ...
        torch.nn.utils.clip_grad_norm_(agent.actor_local.parameters(), 1, 2)
        pl = policy_loss.cpu().detach().item()
        cl = critic_loss.cpu().detach().item()
        assert pl < 1.0, "Policy Loss blowing Up"
